...
def get_repo(repo_owner: str, repo_name: str, db: Session = Depends(get_session)):
    try:
        result = RepoService(db).get_repo(
...
def get_commits(repo_owner: str, repo_name: str, db: Session = Depends(get_session)):
    try:
        return RepoService(db).get_commits(
            f"https://github.com/{repo_owner}/{repo_name}"
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def get_commit(
    repo_owner: str, repo_name: str, commit_sha: str, db: Session = Depends(get_session)
):
    try:
        return RepoService(db).get_commit(
