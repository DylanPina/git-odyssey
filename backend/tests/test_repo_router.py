...
class RepoRouterTests(unittest.TestCase):
    ...
    def test_get_repo_skips_auto_ingest_when_disabled(self) -> None:
        current_user = User(
            id=1,
            username="local-user",
            email="local@gitodyssey.app",
        ...
        repo_service = Mock()
        repo_service.get_repo.return_value = Mock(reindex_required=True)
        ingest_service = Mock()
        ingest_service.resolve_repo_path.return_value = "/tmp/example-project"
        ingest_service.ingest_repo = AsyncMock(return_value="/tmp/example-project")
        ...
        ingest_service.ingest_repo.assert_not_called()
