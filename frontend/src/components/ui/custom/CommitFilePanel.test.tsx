...
describe("CommitFilePanel", () => {
  ...
  it("injects the original-side selection from the context menu action", async () => {
    ...
    await act(async () => {
      ...
      await monacoHarness.originalEditor
        ?.__getActions()[0]
        .run(monacoHarness.originalEditor);
    });
  ...
  it("does nothing for empty or whitespace-only context-menu selections", async () => {
    ...
    await act(async () => {
      ...
      await monacoHarness.modifiedEditor
        ?.__getActions()[0]
        .run(monacoHarness.modifiedEditor);
  ...
  it("uses a 10-line default hidden unchanged region context", async () => {
    render(
      <CommitFilePanel
        repoPath="/Users/dillonpina/Documents/code/git-odyssey"
        viewerId="review:test"
    ...
    await waitFor(() => {
      expect(monacoHarness.lastOptions).toMatchObject({
  ...
  it("allows overriding the hidden unchanged region context lines", async () => {
    render(
      <CommitFilePanel
        repoPath="/Users/dillonpina/Documents/code/git-odyssey"
        viewerId="review:test"
