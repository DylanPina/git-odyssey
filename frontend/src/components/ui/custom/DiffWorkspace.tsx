...
function resolveResizablePanelMaxWidth({
	...
	return Math.max(minWidth, availableWidth - minContentWidth - otherPanelWidth);
}
	...
	return (
		...
		>
			...
			<InputGroupInput
				...
				className={cn(
					"text-sm",
					isCompactChrome ? "px-1.5 py-3" : "px-2 py-3.5",
				)}
			/>
		...
		<button
			...
			onPointerDown={(event) => {
				...
				const handlePointerMove = (moveEvent: PointerEvent) => {
					...
					currentDrag.latestWidth = clampPanelWidth(
						nextWidth,
						minWidth,
						maxWidth,
					);
			...
			onKeyDown={(event) => {
				if (event.key === "ArrowLeft") {
					...
					onWidthChange(
						clampPanelWidth(
							currentWidth - DESKTOP_RESIZE_STEP,
							minWidth,
							maxWidth,
						),
					);
...
>(function DiffWorkspace(
	{
		...
		emptyTitle,
		emptyDescription,
		diffContextLines = DEFAULT_DESKTOP_REPO_SETTINGS.contextLines,
		summaryActions,
		rightRail,
	...
	const [selectedCodeMatchId, setSelectedCodeMatchId] = useState<string | null>(
		null,
	);
	const [activeCodeMatchId, setActiveCodeMatchId] = useState<string | null>(
		null,
	);
	...
	const [desktopWorkspaceWidth, setDesktopWorkspaceWidth] = useState<
		number | null
	>(null);
	...
	const filteredFiles = useMemo(
		() =>
			files.filter((fileChange) => {
				...
				return (
					(codeSearchIndexByPath.get(getFileChangeLabelPath(fileChange))
						?.total ?? 0) > 0
				);
			}),
	...
	const flattenedCodeMatches = useMemo(() => {
		...
		filteredFiles.forEach((fileChange) => {
			...
			matches.push(
				...sortCodeMatches([...fileMatches.original, ...fileMatches.modified]),
			);
		});
	...
	const jumpToSelectedCodeMatch = useCallback(() => {
		...
		const nextMatch = selectedCodeMatch ?? flattenedCodeMatches[0] ?? null;
		if (nextMatch) {
			navigateToCodeMatch(nextMatch, { focusEditor: true });
		}
	}, [flattenedCodeMatches, navigateToCodeMatch, selectedCodeMatch]);

	const clearCodeSearch = useCallback((input?: HTMLInputElement | null) => {
		shouldAutoNavigateToCodeMatchRef.current = false;
		setCodeQuery("");
		setSelectedCodeMatchId(null);
		setActiveCodeMatchId(null);
	...
	const isDesktopFileTreeExpanded =
		isDesktopFileTreeVisible && !isFileTreeCollapsed;
	const isDesktopRightRailVisible = Boolean(
		rightRail || rightRailCollapsedSummary,
	);
	...
	const codeSearchShortcutLabel = useMemo(
		() => getCodeSearchShortcutLabel(),
		[],
	);
	...
	const hasVisibleDiffPanels = filteredFiles.length > 0;
	const areAllVisibleDiffPanelsExpanded =
		hasVisibleDiffPanels &&
		filteredFiles.every(
			(fileChange) => expanded[getFileChangeLabelPath(fileChange)] ?? true,
		);
		...
		<Tooltip>
			<TooltipTrigger asChild>
				<Button
					variant="toolbar"
					size="sm"
					className="shrink-0 rounded-[16px] px-3 text-[12px] [&_svg:not([class*='size-'])]:size-3.5"
	...
	const expandCollapseAllToggle = hasFiles ? (
		<Tooltip>
			<TooltipTrigger asChild>
				<Button
					variant="toolbar"
					...
					className="shrink-0 rounded-[16px] px-3 text-[12px] [&_svg:not([class*='size-'])]:size-3.5"
					aria-label={
						areAllVisibleDiffPanelsExpanded
							? "Collapse all files"
							: "Expand all files"
					}
					disabled={!hasVisibleDiffPanels}
					onClick={() =>
						setExpanded((prev) => {
							const next = { ...prev };

							filteredFiles.forEach((fileChange) => {
								next[getFileChangeLabelPath(fileChange)] =
									!areAllVisibleDiffPanelsExpanded;
							});
	...
	const workspaceHeader = (
		<div className="flex flex-col gap-2 xl:flex-row xl:items-center xl:justify-between">
			...
			{hasFiles ? (
				<div className="flex w-full min-w-0 items-center gap-2 xl:max-w-[40rem]">
					...
					>
						...
						>
							<CommitFileTree
								...
								bottomAction={fileTreeBottomAction}
							...
							>
								...
								>
									{filteredFiles.length > 0 ? (
										<div
											className={cn(
												isCompactChrome ? "space-y-2" : "space-y-4",
											)}
										>
							...
							>
								...
								!isRightRailFullscreen ? (
									<DiffWorkspaceResizeHandle
										...
										onWidthChange={
											desktopResize.rightRail.onPreferredWidthChange
										}
									/>
								) : null}
