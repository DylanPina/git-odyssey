...
type DiffHeaderIconButtonProps = {
  label: string;
  tooltip?: ReactNode;
  children: ReactNode;
  className?: string;
...
function DiffHeaderIconButton({
  ...
  pressed,
  onClick,
}: DiffHeaderIconButtonProps) {
  const button = (
    <Button
      type="button"
      variant={variant}
      size="toolbar-icon"
      className={cn("size-8 [&_svg:not([class*='size-'])]:size-3.5", className)}
      aria-label={label}
...
export function CommitFilePanel({
  ...
  const panelSelectionClass = isSelected
    ...
    : isSearchContextTarget
      ...
      : undefined;
  const nextDiffMode = diffMode === "side-by-side" ? "inline" : "side-by-side";
  const diffModeButtonLabel = `Diff mode: ${diffMode}. Switch to ${nextDiffMode}.`;
  const summaryButtonLabel = summaryLoading
    ? "Summarizing file change"
    : fileSummary?.text
      ? isFileSummaryOpen
        ? "Hide summary"
        : "View summary"
      ...
      <div className="sticky top-0 z-20">
        ...
        >
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            {showDiffModeToggle ? (
              <DiffHeaderIconButton
                label={diffModeButtonLabel}
                tooltip={`Switch to ${nextDiffMode} diff`}
                onClick={(event) => {
                  event.stopPropagation();
                  if (onDiffModeChange) {
                    onDiffModeChange(nextDiffMode);
                    return;
                  }
                  setUncontrolledDiffMode(nextDiffMode);
                }}
              >
            ...
            {activeSelection && onInjectSelection ? (
              <DiffHeaderIconButton
                label={`Add selected code to chat (${selectionLabel})`}
            ...
            {canShowFileSummaryControls ? (
              <DiffHeaderIconButton
                ...
                pressed={fileSummary?.text ? isFileSummaryOpen : undefined}
              >
                {summaryLoading ? (
