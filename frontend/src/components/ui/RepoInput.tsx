...
export function RepoInput() {
  ...
  function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    try {
      ...
      form.setError("githubUrl", { message: "Please enter a valid repository format" });
      setIsLoading(false);
      return;
    }
              ...
              <div className="relative">
                <FormControl>
                  <Input
                    ...
                    className="pl-4 pr-32 h-14 bg-white/10 backdrop-blur-sm border-white/20 text-white placeholder:text-white/50 focus:border-white/40 focus:ring-2 focus:ring-white/20 rounded-xl"
                    disabled={isLoading}
                  />
                </FormControl>
                <Button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 text-white !bg-neutral-800 text-black hover:bg-white/90 disabled:opacity-50 disabled:cursor-not-allowed rounded-lg font-medium transition-all duration-200"
                >
                  {isLoading ? (
