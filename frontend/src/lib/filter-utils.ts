 ...
 */
export function filterCommits(commits: Commit[], filters: FilterFormData, branches: Branch[]): Commit[] {
  ...
  return commits.filter((commit) => {
    ...
    const messageMatch =
      !filters.message || commit.message.toLowerCase().includes(filters.message.toLowerCase());
    ...
    const commitMatch =
      !filters.commit || commit.sha.toLowerCase().includes(filters.commit.toLowerCase());
    ...
    const fileMatch =
      ...
      commit.file_changes.some(
        (fileChange: FileChange) =>
          fileChange.path.toLowerCase().includes(filters.file.toLowerCase())
      );

    // Filter by summary (case-insensitive)
    const summaryMatch =
      !filters.summary || commit.summary?.toLowerCase().includes(filters.summary.toLowerCase());

    // Filter by date range
    const dateMatch = filterByDateRange(commit.time, filters.startDate, filters.endDate);

    return messageMatch && branchMatch && commitMatch && fileMatch && summaryMatch && dateMatch;
  });
}
 ...
 */
function filterByBranch(commitSha: string, branchName: string, branches: Branch[]): boolean {
  if (!branchName) {
